  ...
  return (
    <div className='fixed inset-0 z-50 overflow-y-auto'>
      <div className='flex items-end justify-center min-h-screen px-2 sm:px-4 pt-4 pb-4 sm:pb-20 text-center sm:block sm:p-0'>
        ...
        {/* Modal panel */}
        <div
          className={`inline-block w-full max-w-sm sm:max-w-md md:max-w-2xl lg:max-w-4xl xl:max-w-5xl p-3 sm:p-6 my-4 sm:my-8 overflow-hidden text-left align-middle transition-all transform ${STYLES.dark.background.darkest} shadow-xl rounded-t-xl sm:rounded-lg`}
        >
          {/* Header */}
          <div className='flex flex-col sm:flex-row sm:items-start justify-between mb-4 sm:mb-6'>
            <div className='flex items-center space-x-3 sm:space-x-4 mb-3 sm:mb-0'>
              <div className='w-12 h-12 sm:w-16 sm:h-16 bg-red-100 rounded-full flex items-center justify-center flex-shrink-0'>
                <User
                  className={`w-6 h-6 sm:w-8 sm:h-8 text-[${STYLES.dark.accent.color}]`}
                />
              </div>
              <div className='min-w-0 flex-1'>
                <h2
                  className={`text-lg sm:text-xl lg:text-2xl font-bold ${STYLES.dark.text.primary} truncate`}
                >
                  {candidate.name}
                </h2>
                <a
                  href={`mailto:${candidate.email}`}
                  className={`text-red-600 flex items-center mt-1 text-sm sm:text-base gap-1`}
                >
          ...
          {/* Footer Actions */}
          <div
            className={`flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 mt-6 sm:mt-8 pt-4 sm:pt-6 border-t ${STYLES.dark.border.light}`}
          >
            <button
              onClick={handleClose}
