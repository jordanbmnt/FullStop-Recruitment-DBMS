...
export const DetailsTab = ({ candidate, isFetching,
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      <h3 className="text-base sm:text-lg font-semibold text-gray-100 mb-3 sm:mb-4">Available Documents</h3>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-3 sm:space-y-0">
          <div className="flex items-center space-x-3">
            <div className={`w-8 h-8 sm:w-10 sm:h-10 ${candidate.fileInfo ? "bg-red-100" : STYLES.dark.background.secondary} rounded-lg flex items-center justify-center flex-shrink-0`}>
              <span className={`${candidate.fileInfo ? `text-[${STYLES.dark.accent.color}]` : "text-gray-300"} font-semibold text-xs sm:text-sm`}>CV</span>
            </div>
            <div className="min-w-0">
              <h4 className="font-medium text-gray-100 text-sm sm:text-base">Curriculum Vitae</h4>
              <p className="text-xs sm:text-sm text-gray-400">PDF Document</p>
            </div>
          </div>
            ...
            candidate.fileInfo ? (
              <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                ...
                <button
                  ...
                  className={`flex items-center justify-center space-x-2 px-3 py-2 text-xs sm:text-sm font-medium ${isFetching ?
                    ...
                    `text-gray-50 ${STYLES.dark.background.tertiary} hover:bg-black hover:${STYLES.dark.border.medium}`
                    } rounded-md transition-colors`}
                >
              ...
              <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                <button
                  ...
                  className={`flex items-center justify-center space-x-2 px-3 py-2 text-xs sm:text-sm font-medium text-gray-50 ${STYLES.dark.background.secondary} rounded-md hover:${STYLES.dark.background.tertiary} transition-colors`}
                >
      ...
      <div className="space-y-3 sm:space-y-4">
        <h3 className="text-base sm:text-lg font-semibold text-gray-100 mb-3 sm:mb-4">Cover Letter</h3>
        <div className={`border ${STYLES.dark.border.light} rounded-lg p-3 sm:p-4`}>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-3 sm:space-y-0">
            <div>
              {candidate.coverLetter && (
                <p className="text-gray-400 leading-relaxed text-sm sm:text-base">{candidate.coverLetter}</p>
              )}
            </div>
          </div>
        </div>
      ...
      {candidate.previousJobReasons && (
        <div className="space-y-3 sm:space-y-4">
          <h3 className="text-base sm:text-lg font-semibold text-gray-100 mb-3 sm:mb-4">Reason for leaving last position</h3>
          <div className={`border ${STYLES.dark.border.light} rounded-lg p-3 sm:p-4`}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-3 sm:space-y-0">
              <div>
                <p className="text-gray-400 leading-relaxed text-sm sm:text-base">{candidate.previousJobReasons}</p>
              </div>
            </div>
          </div>
        </div>
