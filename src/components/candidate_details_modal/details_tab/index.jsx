...
export const DetailsTab = ({ candidate, isFetching, searchResult }) => {
  ...
  return (
    <div className='space-y-3 sm:space-y-4'>
      <h3
        className={`text-base sm:text-lg font-semibold ${STYLES.dark.text.primary} mb-3 sm:mb-4`}
      >
        Available Documents
      </h3>
      ...
      {/* CV Section */}
      <div
        className={`border ${STYLES.dark.border.light} rounded-lg p-3 sm:p-4 transition-all`}
      >
        <div className='flex flex-col sm:flex-row sm:items-center justify-between space-y-3 sm:space-y-0'>
          <div className='flex items-center space-x-3'>
            <div
              className={`w-8 h-8 sm:w-10 sm:h-10 ${
                candidate.fileInfo
                  ? "bg-red-100"
                  : STYLES.dark.background.secondary
              } rounded-lg flex items-center justify-center flex-shrink-0`}
            >
              <span
                className={`${
                  candidate.fileInfo
                    ? `text-[${STYLES.dark.accent.color}]`
                    : STYLES.dark.text.secondary
            ...
            <div className='min-w-0'>
              <h4
                className={`font-medium ${STYLES.dark.text.secondary} text-sm sm:text-base`}
              >
                Curriculum Vitae
              </h4>
              <p className={`text-xs sm:text-sm ${STYLES.dark.text.tertiary}`}>
                PDF Document
              </p>
            </div>
          </div>
          ...
          {candidate.fileInfo && searchResult && searchResult.pdfElement ? (
            ...
            <div className='flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2'>
              <button
                onClick={() => {
                  window.location = `mailto:${candidate.email}?subject=Missing+CV+in+our+database`;
                }}
      ...
      <div className='space-y-3 sm:space-y-4'>
        ...
        <div
          className={`border ${STYLES.dark.border.light} rounded-lg p-3 sm:p-4`}
        >
          <div className='flex flex-col sm:flex-row sm:items-center justify-between space-y-3 sm:space-y-0'>
            <div>
              {candidate.coverLetter && (
                <p
                  className={`${STYLES.dark.text.tertiary} leading-relaxed text-sm sm:text-base`}
                >
                  {candidate.coverLetter}
                </p>
              )}
