...
export const SearchBar = () => {
  ...
  const [searchQueryLimit, setSearchQueryLimit] = useState(12);
  ...
  useEffect(() => {
    if (searchQuery) {
      try {
        ...
        let url = `${ROOT_PARAM}?name=${searchQuery.toLowerCase()}&email=${searchQuery.toLowerCase()}&limit=${searchQueryLimit}`;
  ...
  const handleSearch = (event) => {
    ...
    if (event.code === "Enter" || event.type === "click") {
      ...
      else setSearchQueryParams(null);
  ...
  return (
    <div className="mx-auto p-6">
      <div className="bg-white rounded-xl shadow-lg p-8">
        ...
        {/* Main Search Bar */}
        <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 mb-6" onMouseLeave={() => {
